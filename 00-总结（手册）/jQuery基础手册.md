...
#### 后代选择器 
    $("div  p")
#### 相邻兄弟选择器 [不会向前选择]
    $("div + p")  //选择所有紧接在“div”元素后的“p”元素
#### 一般兄弟选择器 [不会向前选择]
    $("div ~ p")  //匹配“div”元素之后的所有 兄弟元素。具有相同的父元素，并匹配过滤“p”选择器

## jQuery 基本筛选 选择器 
> [选择某个元素里的哪个元素][根据元素选元素]  
>> 举例: $(.class:first) 找到第一个类
...
## jQuery 内容筛选
    $(":contains('?')") 选择所有包含指定内容的元素      		 通过/根据 内容来找
    $(":has(?元素)") 选择元素中至少包含指定选择器的元素          通过/根据 元素来找
    $("a:parent") 选择含有子元素或者文本的元素                 有内容的a会被选中
    $("a:empty") 选择没有含子元素或文本的元素                  没有内容的a会被选中
...
## jQuery 选择器之 属性选择器
    $('div[name=p1]')			属性name=p1的div元素
    $('div[p2]')				有属性p2的div元素
    $('div[name|="-"]')			有属性name中的值只包含一个连字符“-”的div元素
    $('div[name~="a"]') 	    有属性name中的值包含一个连字符“空”和“a”的div元素
    $('div[name^=imooc]') 	    属性name的值是用imooc开头的
    $('div[name$=imooc]')  	    属性name的值是用imooc结尾的
    $('div[name*="test"]')		有属性name中的值包含一个test字符串的div元素
    $('div[testattr!="true"]')  有属性testattr中的值没有包含"true"的div
...
# 遍历
    children()	    $("A").children()		抓取所有A元素的 所有子元素
    find()			$("A").find()			抓取所有A元素的 所有后代元素
    parent()		$("A").parent()			抓取所有A元素的 所有父级元素
    parents() 		$("A").parents()		抓取所有A元素的 所有祖辈元素
    closest()		$("A").closest()		抓取所有A元素 包括A元素和一个(符合的)祖辈元素 			
    next() 		    $("A").next()			抓取所有A元素的 同辈下一个元素
    prev() 			$("A").prev()			抓取所有A元素的 同辈上一个元素
    siblings()		$("A").siblings()		抓取所有A元素的 同辈的所有元素
    add() 
