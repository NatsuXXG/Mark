                  ...
                  <div class="type-h-wrap">
                    ...
                    <div class="type-h-right">
                      <p class="type-h-odd">
                        ...
                        <span class="onlywin" :class="{'click-hook': sport.Match_BzM > 0}" @click="submitOrder(sport,sport.Match_BzM,sport.Match_Master,'主队-独赢', 'Match_BzM')">{{sport.Match_BzM > 0 ? sport.Match_BzM : ''}}</span>
                        <span class="pankou-wrap">
                          <span class="pankou" v-if="sport.Match_ShowType === 'H'">{{pankouFilter(sport.Match_RGG)}}</span>
                          <span class="h-ho" :class="{'click-hook': sport.Match_Ho > 0}" @click="submitOrder(sport,sport.Match_Ho,sport.Match_Master,'主队-让球', 'Match_Ho')">{{sport.Match_Ho>0 ? sport.Match_Ho : ''}}</span>
                        </span>
                        <span class="dx-wrap">
                          <span class="dx-pkou">大{{pankouFilter(sport.Match_DxGG)}}</span>
                          <span class="dx-x" :class="{'click-hook': sport.Match_DxDpl > 0}" @click="submitOrder(sport,sport.Match_DxDpl,'大','主队-大小', 'Match_DxDpl')">{{sport.Match_DxDpl > 0 ? sport.Match_DxDpl: ''}}</span>
                        </span>
                        <span class="ds-d-wrap" :class="{'click-hook': sport.Match_DsDpl > 0}" @click="submitOrder(sport,sport.Match_DsDpl,'单','主队-单双', 'Match_DsDpl')">{{sport.Match_DsDpl > 0 ? '单' + sport.Match_DsDpl : ''}}</span>
                      </p>
                      <p class="type-h-odd">
                        <strong class="type-name">客</strong>
                        <span class="onlywin" :class="{'click-hook': sport.Match_BzG > 0}" @click="submitOrder(sport,sport.Match_BzG,sport.Match_Guest,'客队-独赢', 'Match_BzG')">{{sport.Match_BzG>0?sport.Match_BzG:''}}</span>
                        <span class="pankou-wrap">
                          <span class="pankou" v-if="sport.Match_ShowType === 'C'">{{pankouFilter(sport.Match_RGG)}}</span>
                          <span class="h-ho" :class="{'click-hook': sport.Match_Ao > 0}" @click="submitOrder(sport,sport.Match_Ao,sport.Match_Guest,'客队-让球', 'Match_Ao')">{{sport.Match_Ao>0?sport.Match_Ao:''}}</span>
                        </span>
                        <span class="dx-wrap">
                          <span class="dx-pkou">小{{pankouFilter(sport.Match_DxGG)}}</span>
                          <span class="dx-x" :class="{'click-hook': sport.Match_DxXpl > 0}" @click="submitOrder(sport,sport.Match_DxXpl,'小','客队-大小', 'Match_DxXpl')">{{sport.Match_DxXpl>0?sport.Match_DxXpl:''}}</span>
                        </span>
                        <span class="ds-s-wrap" :class="{'click-hook': sport.Match_DsSpl > 0}" @click="submitOrder(sport,sport.Match_DsSpl,'双','客队-单双', 'Match_DsSpl')">{{sport.Match_DsSpl>0?'双'+sport.Match_DsSpl:''}}</span>
                      </p>
                    </div>
                  </div>
                </div>
