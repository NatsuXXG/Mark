  ...
  export default {
    ...
    data () {
      return {
        ...
        posturl: '/api/member/lottery/order_lottery.php?gType=9',
        minMoney: 0, // 最低下注金额
    ...
    methods: {
      // 获取球名跟号码
      _getballs () {
        this.$http.get('/api/member/lottery/odds.php?gType=9').then((res) => {
          if (res.status === 200) {
      ...
      _getRecords () {
        this.$http.get('/api/member/lottery/lottery_auto.php?gType=9').then((res) => {
          if (res.status === 200) {
      ...
      // 获取历史开奖结果
      _getHistoryRecords () {
        this.$http.get('/api/member/lottery/lotResult.php?type=9').then((res) => {
          if (res.status === 200) {
